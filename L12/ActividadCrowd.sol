...
contract CrowdFunding {
    ...
    function votarPeticionGasto(uint IDpeticion, bool aprobar) public {
        ...
        require(!peticionesDeGasto[IDpeticion].votantes[msg.sender] ,"Ya has votado.");
        require(participaciones[msg.sender] > 0 ,"No estas en la lista de participantes.");
        peticionesDeGasto[IDpeticion].votantes[msg.sender] = aprobar;
        peticionesDeGasto[IDpeticion].votosRealizados++;
        if (aprobar) peticionesDeGasto[IDpeticion].votosPositivos++;
    }

    function resolverPeticion(uint index) public only_manager {

        require(peticionesDeGasto[index].estado == EstadoPeticion.Abierta, "Peticion ya cerrada.");
        if (peticionesDeGasto[index].votosRealizados >= totalParticipaciones/2) {
